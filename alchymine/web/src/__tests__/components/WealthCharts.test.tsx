...
describe('Wealth Page Charts', () => {
  ...
  it('shows the avalanche method label', () => {
    render(<WealthPage />);
    expect(screen.getByText('Avalanche Method (highest rate first)')).toBeInTheDocument();
  });
  ...
  it('displays the not-financial-advice disclaimer', () => {
    render(<WealthPage />);
    expect(screen.getByText('Not financial advice. All strategies require professional review.')).toBeInTheDocument();
  });
});
