...
const DEMO_DEBTS: DebtItem[] = [
  { name: 'Student Loans', balance: 12400, total: 35000, rate: 5.5, monthsRemaining: 36 },
  { name: 'Credit Card', balance: 2800, total: 8000, rate: 19.9, monthsRemaining: 8 },
  { name: 'Car Loan', balance: 8200, total: 18000, rate: 4.2, monthsRemaining: 24 },
  { name: 'Personal Loan', balance: 1200, total: 5000, rate: 7.8, monthsRemaining: 4 },
];
  ...
  return (
    <div data-testid="debt-payoff-timeline">
            ...
            <div key={debt.name}>
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-text/80">{debt.name}</span>
                  <span className="text-xs text-red-400/70">{debt.rate}% APR</span>
                </div>
      ...
      <div className="mt-4 flex items-center justify-between text-xs text-text/40">
        <span>
          Total remaining: {formatCurrency(debts.reduce((sum, d) => sum + d.balance, 0))}
        </span>
        <span>
          Paid off: {formatCurrency(debts.reduce((sum, d) => sum + (d.total - d.balance), 0))}
        </span>
      </div>
    </div>
  );
...
function BudgetBreakdown({
  ...
  const savingsRate = savingsCategory ? (savingsCategory.amount / income) * 100 : 0;
    ...
    <div data-testid="budget-breakdown">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-bold text-primary">{formatCurrency(income)}</span>
        </div>
      </div>

      {/* Stacked bar */}
      <div className="h-8 rounded-full overflow-hidden flex mb-4" role="img" aria-label="Budget allocation bar">
        {categories.map((cat) => {
      ...
      <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-green-400">{savingsRate.toFixed(0)}%</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div data-testid="net-worth-tracker">
      ...
      <div className="space-y-3 mb-4">
        ...
        <div>
          <div className="flex justify-between text-xs mb-1">
            ...
            <span className="text-text/60">{formatCurrency(totalLiabilities)}</span>
          </div>
          <div className="h-6 bg-surface rounded-lg overflow-hidden">
            <div
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <h4 className="text-xs uppercase tracking-wider text-text/40 mb-2">Assets</h4>
          <div className="space-y-2">
            {assets.map((a) => (
              <div key={a.name} className="flex justify-between items-center text-sm">
                <span className="text-text/60">{a.name}</span>
                <span className="text-green-400/80 font-medium">{formatCurrency(a.value)}</span>
              </div>
            ))}
          </div>
        </div>
        <div>
          <h4 className="text-xs uppercase tracking-wider text-text/40 mb-2">Liabilities</h4>
          <div className="space-y-2">
            {liabilities.map((l) => (
              <div key={l.name} className="flex justify-between items-center text-sm">
                <span className="text-text/60">{l.name}</span>
                <span className="text-red-400/80 font-medium">-{formatCurrency(l.value)}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-text/80">{lever}</span>
                  </div>
                  <span className="text-xs text-text/40">
  ...
  return (
    <main className="min-h-screen px-4 sm:px-6 lg:px-8 py-8">
      <div className="max-w-5xl mx-auto">
        ...
        <section className="mb-12" aria-labelledby="financial-dashboard-heading">
          <h2 id="financial-dashboard-heading" className="text-2xl font-bold mb-6 flex items-center gap-3">
            <span className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-xl" aria-hidden="true">
              {'\u{1F4CA}'}
            </span>
            Financial Dashboard
          ...
          <p className="text-text/50 text-sm mb-6">
            Sample visualizations showing how your financial data will be displayed.
            All calculations use standard deterministic formulas.
          </p>
          ...
          <div className="space-y-6">
            ...
            {/* Budget Breakdown */}
            <Card title="">
              <BudgetBreakdown income={DEMO_BUDGET.income} categories={DEMO_BUDGET.categories} />
            </Card>

            {/* Net Worth Tracker */}
            <Card title="">
              <NetWorthTracker assets={DEMO_NET_WORTH.assets} liabilities={DEMO_NET_WORTH.liabilities} />
            </Card>
          </div>
        </section>
