...
const SYSTEMS = ['intelligence', 'healing', 'wealth', 'creative', 'perspective'];
const ENTRY_TYPES = ['reflection', 'insight', 'gratitude', 'intention', 'freeform'];
    ...
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      {stats && (
        ...
        >
          ...
          <StatCard
            label="Avg Mood"
            value={stats.average_mood !== null ? `${stats.average_mood.toFixed(1)} ${moodEmoji(stats.average_mood)}` : '—'}
          />
      ...
      {showForm && (
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '0.75rem' }}>
            ...
            <div>
              <label style={labelStyle}>Mood (1-10): {formMood} {moodEmoji(formMood)}</label>
              <input
                type="range"
                min={1}
                max={10}
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1rem' }}>
              <h2 style={{ fontSize: '1.25rem', fontWeight: 600 }}>{selectedEntry.title}</h2>
              <button
                onClick={() => setSelectedEntry(null)}
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
              <span style={{ ...tagStyle, background: systemColor(selectedEntry.system) + '33', color: systemColor(selectedEntry.system) }}>
                {selectedEntry.system}
              </span>
              <span style={{ ...tagStyle, background: '#374151', color: '#d1d5db' }}>
                {selectedEntry.entry_type}
              </span>
              {selectedEntry.mood_score !== null && (
                <span style={{ ...tagStyle, background: '#374151', color: '#d1d5db' }}>
                  Mood: {selectedEntry.mood_score} {moodEmoji(selectedEntry.mood_score)}
                </span>
              )}
            </div>
            ...
            {selectedEntry.tags.length > 0 && (
              <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', flexWrap: 'wrap' }}>
                {selectedEntry.tags.map((tag) => (
                  <span key={tag} style={{ ...tagStyle, background: '#1f2937', color: '#9ca3af', fontSize: '0.75rem' }}>
                    #{tag}
                  </span>
                ))}
              </div>
      ...
      {!loading && entries.length === 0 && (
        <div style={{ textAlign: 'center', padding: '3rem', color: '#6b7280' }}>
          <p style={{ fontSize: '1.25rem', marginBottom: '0.5rem' }}>No journal entries yet</p>
        ...
        {entries.map((entry) => (
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
              <h3 style={{ fontWeight: 600, fontSize: '1rem', marginBottom: '0.25rem' }}>{entry.title}</h3>
              {entry.mood_score !== null && (
                <span style={{ fontSize: '1.25rem' }}>{moodEmoji(entry.mood_score)}</span>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
              <span style={{ ...tagStyle, background: systemColor(entry.system) + '33', color: systemColor(entry.system), fontSize: '0.7rem' }}>
                {entry.system}
              </span>
              <span style={{ ...tagStyle, background: '#374151', color: '#9ca3af', fontSize: '0.7rem' }}>
                {entry.entry_type}
              </span>
