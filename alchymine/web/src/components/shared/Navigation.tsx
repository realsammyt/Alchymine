...
const NAV_ITEMS: NavItem[] = [
  { name: 'Dashboard', href: '/dashboard', icon: 'home', label: 'Dashboard overview' },
  { name: 'Intelligence', href: '/intelligence', icon: 'brain', label: 'Personalized Intelligence system' },
  { name: 'Healing', href: '/healing', icon: 'leaf', label: 'Ethical Healing system' },
  { name: 'Wealth', href: '/wealth', icon: 'chart', label: 'Generational Wealth system' },
  { name: 'Creative', href: '/creative', icon: 'palette', label: 'Creative Development system' },
  { name: 'Perspective', href: '/perspective', icon: 'telescope', label: 'Perspective Enhancement system' },
];
...
function NavIcon({ icon, className = '' }: { icon: string; className?: string }) {
  ...
  switch (icon) {
    case 'home':
      return (
        <svg className={baseClass} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
  ...
  return (
    <>
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 bg-surface/95 backdrop-blur-xl border-b border-white/5 z-50">
        ...
        {mobileMenuOpen && (
          <nav className="px-4 pb-4 animate-fade-in" aria-label="Main navigation">
            ...
            return (
              <li key={item.name}>
                ...
                >
                  ...
                  <span className="truncate max-w-[48px]">{item.name === 'Dashboard' ? 'Home' : item.name.slice(0, 5)}</span>
                </Link>
              </li>
            );
          })}
