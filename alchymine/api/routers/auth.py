...
async def register(
    ...
    if body.promo_code != settings.signup_promo_code:
        # Atomically increment uses_count and verify all constraints in one UPDATE.
        # The WHERE clause enforces is_active, uses_count < max_uses, and expiry
        # atomically to prevent race conditions between concurrent registrations.
        ...
        invite_code_row = update_result.scalar_one_or_none()
        if invite_code_row is None:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Invalid invitation code — it may be expired, exhausted, or inactive",
            )
