...
class ChatMessage(Base):
    ...
    Stores the conversation history so the assistant can provide contextual
    ...
    system_context: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="e.g. 'healing', 'wealth' — which system specialist"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), index=True
    )
    ...
    def __repr__(self) -> str:
        return f"<ChatMessage id={self.id!r} user_id={self.user_id!r} role={self.role!r}>"
