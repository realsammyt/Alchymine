...
| 5 | #257 | `practice` coach scope, context builder, safety gates, `request_id` attribution | **The live chat endpoint.** Highest of the five. | Remove `"practice"` from `_VALID_SYSTEM_KEYS`: the scope 422s, every other chat scope is untouched. Cheap kill switch, keep it in mind during review. **One deliberate exception, added in review:** the crisis gate reads `PRACTICE_SYSTEM_KEY` directly and runs *before* the validity check, so a crisis-severity message still gets resources on a killed scope rather than a 422. Answering somebody in crisis with a schema error because an operator disabled a feature is the failure mode that gate exists to prevent, and the resource stream costs nothing. Pinned by `TestKillSwitch`. |
