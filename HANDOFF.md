...
**Last updated:** 2026-08-19 by Claude (autonomous resilience/correctness session complete: #297/#313/#292/#314/#312/#269/#308 shipped and closed, 6 PRs merged, #304 cut with a written line, live verification on a production build)
...
systems + FastAPI + Celery + Next.js), now carrying a generic practice layer:
externally mountable practice packs (license + attribution enforced at load,
revocable by unmount), a practice log, a deterministic ecology recommender, a
daily protocol surface, and an integration loop that turns practice into
tracked capacity change.

Current position: the engine is a substrate for content that stays its
author's. Packs mount from outside the repo, carry their own terms, and leave
cleanly; the product ships only original content. The potential this opens,
staged honestly: today, one person's daily practice loop, live end to end;
next, third-party frameworks arriving as sovereign packs on their authors'
terms; then group-scale practice ecologies, where individual loops compose
into shared protocols and community-visible capacity change. Near-term build
is scoped from the 2026-08-17 deliverability audit gap list (see row 10).
...
| - | ---- | ------ | ------- | ---- |
| 12 | Resilience/correctness session (slices 1-6: chat stream resilience #297+#313, integration merge race+note cap #292, profile create-or-update #314, practice reload hydration #312, ecology settings #269, skill attribution UI #308 + Button ring rider; slice 7 #304 cut with a written line) | Done | 2026-08-19 | 6 PRs squash-merged, every one behind an independent review: #316→aecc821 (+ blocking security PASS), #317→34cae01, #320→ee409b9 (4 review rounds; blocking silent-failure hunt found 2 blockers + 2 regressions in the fix rounds, all fixed red-first), #323→1178923 (REQUEST CHANGES→PASS, retry-unmount race), #327→d102b4d (pre-review WCAG audit, 3 contrast fixes, rings at computed /60), #328→0d3f220. Tests 3480→3581 backend (note: 2026-08-18 close-out figure 3441 was stale; verified 3480 at session start) / 623→725 web (+63 suites). Issues closed with evidence: #269 #292 #297 #308 #312 #313 #314. Live verification on a fresh 0001→0019 Postgres + PRODUCTION web build at 375px + desktop: #314 200-not-500, #312 reload keeps completions, #269 save recomputes protocol live at new size, free-plan crisis gets full resources not 402 (rail held), crisis turn persists and renders after reload, journey/nudge first-visit load. Found live and filed: #329 (dev StrictMode log-hydration give-up, dev-only), #330 (SkillDetailDrawer mounted nowhere — #308's block unreachable until the skills browser lands), #331 (journey chart 31px overflow at 375px, pre-existing PR #309). Follow-ups filed: #318 #319 #321 #322 #324 #325 #326 #329 #330 #331. Slice 7 (#304) deliberately NOT built: content authoring + public-surface shims, cut line on the issue. Awaiting Tyler: 26 new DRAFT strings (5 in PR #320 incl. interrupted-reply + unsaved-history notes, 1 in PR #317 note-cap refusal + cap value 20000, 1 in PR #323 log-read notice, 19 in PR #327 backend+settings UI), #275 ring sweep target is /60 by computation (evidence on the issue). |
...
| 9 | Practice layer (epic #251: pack schema v2 + loader + external mounts, migration 0018 + practice log, deterministic ecology recommender, daily protocol + integration loop surfaces, practice coach scope) | Done | 2026-08-14 | All 6 slices merged (PRs #258 design, #259, #260, #261, #262, #277), issues #252-#257 closed with evidence. Tests 2724→3161 backend, 411→526 web, zero regressions. Every merge behind independent review; blocking security pass on slices 1/2/4/5 (all PASS); mutation-tested recommender coverage (slice 3); WCAG 2.1 AA audit on slice 4 (2 blockers found and fixed). Licensing rail held: generic engine only, original 10-practice example pack, zero third-party frameworks or vocabulary in-repo; branded packs mount later via PRACTICE_PACK_DIRS with required license+attribution metadata. Design doc: docs/plans/2026-08-14-practice-layer.md (28-entry decision register). Awaiting Tyler: external content-licensing and partnership conversations (tracked off-repo), ~110 DRAFT strings across slices 3-5 (inventories in PR bodies; crisis-frame wrapper lines in PR #277 flagged for priority read), example-pack content review. Follow-ups #263-#280 |
| 10 | Deliverability audit of the practice layer's public claims (4 agents: runtime, pack contract, provenance, claims) | Done | 2026-08-17 | Provenance: product code, bundled pack and prompts carry zero third-party names or vocabulary; this file was the one leak and is now redacted (detail moved off-repo). Pack contract proven against the live loader: a synthetic 7-practice pack with a dependency graph loads, progresses and recommends; license/attribution refusals and revocation verified with caveats recorded. Claims audit: 18 of 23 public claims verified as written; 3 softened, 2 overclaims corrected in outreach drafts. Runtime walk complete on real Postgres (0001→0018 applied cleanly): protocol, log, balance/decline behavior, the full intention-to-outcome chain, revocation, library attribution, and both zero-cost chat gates proven in a live browser. Three product bugs found live: the chat page's deep links drop the practice coach scope (web/src/app/chat/page.tsx VALID_SYSTEMS, a sixth sync point slice 5 missed), the UI double-writes integration entries per practice (inflating outcome rows), and crisis resources sit behind the plan gate (a free user in crisis gets a 402 upsell). Gap list feeds the next session: those three first, then pack-declared dimensions (schema 2.1), daily re-engagement nudge, dashboard/journey practice surfaces (#266), CI pack-boundary check, envelope purge on unmount, healing-layer license fields (#265), encryption-key startup check |
| 11 | Quality/safety session (audit gap list + #278 + #263/#279: crisis ungated, deep-link scopes, integration idempotency + migration 0019, SSE framing, safety gates on all six scopes, CI pack-boundary check, envelope purge, encryption-key startup check, healing-skills licensing #265, journey time series #266, daily nudge) | Done | 2026-08-18 | 11 PRs squash-merged, every one behind an independent review (blocking security PASS on slices 1/5/6c; silent-failure hunt on SSE; WCAG code audit on journey): #286→4a1e1e2, #287→de2c92d, #291→b18b8ca, #293→c028b44, #299→0b56cc6, #302→e28d0cf, #298→1a9367e, #301→55e8f1d, #306→17f58a9, #307→dc30e1c, #309→fde6328, #311→3bcaa85. Tests 3161→3441 backend, 526→623 web, zero regressions. Issues closed with evidence: #263 #265 #266 #278 #279 #283 #284 #285 #294 #295 #296 #310. Live UI pass on a fresh 0001→0019 Postgres stack: free-plan crisis gets resources not 402 (API + browser, practice/healing/unscoped), double-save yields one integration row with ciphertext note at rest, journey all four states at 375px + desktop, nudge render/dismiss/zero-request-when-dismissed, deep link lands on practice coach. Migration 0019 DEPLOY ORDER: on any DB that has served POST /practice/integration, run the #290 cleanup (Tyler-reviewed, preview-by-default) BEFORE migrating; 0019 refuses loudly on duplicates and deletes nothing. Awaiting Tyler: #290 cleanup script run, 7 new DRAFT strings (2 crisis general-scope frames in PR #302, 5 nudge strings in PR #311) plus journey-page copy inventory (PR #309), email-nudge channel decision (in-app shipped; Resend parked as spam surface), pack-boundary denylist seed extension (hash workflow in PR #301; Tyler holds the off-repo list). Follow-ups filed: #288 #289 #292 #297 #300 #303 #304 #305 #308 #312 #313 #314 |

**Next action:** (1) Tyler: the epic #251 AWAITING list — external pack-licensing and partnership conversations (tracked off-repo; the design doc's section 8.4 external-pack contract is the technical basis), DRAFT copy review (inventories in PR #262/#277 bodies; the practice coach crisis-frame wrapper lines first), and the alchymine-foundations example pack content read (10 practices, speaks in Alchymine's voice); (2) Tyler: epic #221 parked items still open — 14 DRAFT upsell strings (PR #238 body), provisional allowance/budget numbers, provider-console caps, /pricing (#239); (3) Tyler: publish the auto-drafted release when ready to deploy (migrations 0016+0017+0018 run on deploy; 0018 is additive, forward-only in production); (4) Tyler, new from the 2026-08-18 session: run the #290 duplicate-row cleanup before any deploy that includes migration 0019 (cleanup-then-migrate order on any DB that served POST /practice/integration), review the 7 new DRAFT strings (PR #302 crisis frames first, then PR #311 nudge, PR #309 journey copy), decide the email-nudge channel (in-app shipped; Resend parked), extend the pack-boundary denylist seed from the off-repo list (workflow in PR #301); (5) DONE 2026-08-19: #297 #313 #292 #269 #308 #312 #314 all shipped and closed (row 12); (6) Tyler, new from the 2026-08-19 session: review the 26 new DRAFT strings (PR #320 chat resilience notes first, then PR #327 settings surface, PR #317 note-cap refusal, PR #323 log-read notice), confirm or retune integration_note_total_char_cap=20000, and make the #304 calls that gate the healing-skills migration (pack grouping, modality mapping, authored content — cut line on the issue); (7) NEXT session candidates: skills browser wiring #330 (unblocks #308's attribution actually being seen), #275 focus-ring sweep at the computed /60, Stripe/ToS//pricing (#239), journey chart 375px overflow #331, deleted-account token semantics #319, dev StrictMode log-hydration #329.
...
- **2026-08-19** — Resilience/correctness session complete: slices 1-6 merged (chat stream persistence-on-disconnect with BaseException-safe finally + shielded detached fallback + honest event:unsaved signal, interrupted-reply state with Ask again, no more content eviction, crisis frames on _sse_data_frame byte-identical, StrictMode history fix + bounded abort-retry in useApi that fails loud; integration merge row lock forced by EncryptedString + 20000-char note cap refusing not truncating + tail-only dedup; POST /profile create-or-update 200/201 with blocking security PASS; DailyProtocol hydrated from the day's log with no-downgrade + in-place retry merge; GET+PATCH /practice/ecology with fingerprint-nuance clearing + ProtocolSettings disclosure at computed /60 rings; skill attribution block + Button ring rider). Slice 7 (#304) cut with a written line: content authoring + public-surface shims, Tyler-gated. PR #320 took 4 review rounds; the blocking silent-failure hunt caught a give-up path stranding loading forever, a clean done after known double persistence failure, and then a rollback-cancellation regression in the fix itself plus its sibling — all fixed red-first. Live verification on fresh Postgres + production build (dev StrictMode noise filed as #329): crisis rail held for a free account, #312/#314/#269 proven in-browser at 375px + desktop. Tests 3480→3581 / 623→725. Found live: SkillDetailDrawer mounted nowhere (#330), journey chart 375px overflow (#331). 10 follow-up issues filed, 7 closed with evidence.
- **2026-08-18** — Quality/safety session complete: the audit's three live bugs fixed (crisis reachable with zero entitlement checks on every scope incl. unscoped coach — free-plan crisis streams deterministic resources and escapes the 402 structurally; chat deep links keep all six scopes via a single web-side SYSTEM_KEYS enumeration; one integration entry per completion via merge-on-save + unique index in migration 0019, historical cleanup parked as #290), #278 SSE framing fixed per spec (multi data: lines per event, client reassembles at event boundary, all six scopes), #263/#279 shipped (detect_crisis + check_text on all six scopes plus the unscoped coach, per-scope ethics contexts, missing disclaimers appended post-stream never blocked, report-pipeline half filed as #300), audit gap list cleared (CI pack-boundary check with hashed denylist, envelope purge on unmount, encryption-key fail-fast at boot covering API/Celery/CLI, healing-skills licensing #265 with loud external-dir failures and one REST+MCP registry), and the Phase 2 spine started (journey rebuilt as a real time series over practice_log + integration entries with four distinct async states, in-app daily nudge with no dark patterns and per-day zero-request dismissal). 11 PRs, 12 issues closed, tests 3161→3441 / 526→623, live browser verification at 375px + desktop on a fresh Postgres. Three pre-existing bugs found and filed during the live pass (#312 practice reload state, #313 dev StrictMode stranded async, #314 profile POST 500 for registered accounts).
